...
# Here we assume the folder is:
folder = "/mnt/raid/michael/benchmark_tools/20250321_GROK1_CK-MOE-I4F8-AITER-DECODE-ATTN_online"
# Output CSV file name:
output_csv = os.path.join(folder, "20250321_GROK1_CK-MOE-I4F8-AITER-DECODE-ATTN_online_parsed.csv")
...
# Row labels for the two modes
label_aiter = "MI300x-aiter (prefill+decode), dell300x-pla-t10-17"
label_aiter_decode = "MI300x-aiter_decode (decode only), dell300x-pla-t10-17"
...
def build_section(section_title, h100_ref, label_aiter, label_decode):
    ...
    # For each request rate, compute ratio = round((H100_value / measured) * 100) + "%"
    ratio_aiter = []
...
section_e2e = build_section("Median E2E Latency (ms, lower better)", H100_E2E[:5], 
                             "MI300x-aiter (prefill+decode), dell300x-pla-t10-17", 
                             "MI300x-aiter_decode (decode only), dell300x-pla-t10-17")
